...
/* 	RESOLVED USING FOR LOOP */
	...
	// You can define position as THREE.Vector3(x, y, z) or in the following way:
		...
		stepMesh.position.y = verticalStepHeight/2 +riserHeight*i;	// half of height: put it above ground plane
		stepMesh.position.z = 0 +riserDepth*i;			// centered at origin
		scene.add( stepMesh );
		...
		stepMesh.position.y = stepThickness/2 + verticalStepHeight+riserHeight*i;
		// Push step forward by half the depth, minus half the vertical step's thickness
		stepMesh.position.z = horizontalStepDepth/2 - stepHalfThickness +riserDepth*i;
		scene.add( stepMesh );
